#!/usr/bin/env python3
"""
Ultra-careful i18n migration - only replaces text in JSX text nodes.
NEVER touches attributes, tags, or expressions.
"""
import os
import re
...
def migrate_file(filepath):
    ...
    # Only replace German text that is a JSX text node
    # Pattern: >German text< (between JSX tags, not inside JS expressions)
    # We exclude text containing { } < > & \n to avoid breaking expressions
    ...
    # This regex matches text between > and < that doesn't contain special chars
    content = re.sub(r'>([^\n<>{}&]{2,})<', replace_text, content)
    
    # Only replace specific JSX attributes
