...
export default function CalendarPage() {
  ...
  const statusColor = (status: string) => {
    ...
    if (status === "PLANNED") return "bg-primary";
    return "bg-secondary";
  };
  ...
  const weekLabel = (() => {
    ...
    return sameMonth
      ? `${start.toLocaleDateString("de-DE", { day: "numeric" })}.–${end.toLocaleDateString("de-DE", { day: "numeric", month: "long", year: "numeric" })}`
    ...
    return (
      <div>
        <div className="d-grid" style={{ gridTemplateColumns: "repeat(7, 1fr)", gap: "1px", background: "#e2e8f0" }}>
          {weekDays.map((wd) => (
            <div key={wd} className="text-center fw-semibold small py-2 bg-white text-muted">{wd}</div>
          ))}
          {days.map((day, i) => {
            ...
            return (
              ...
              >
                <div className="d-flex justify-content-between align-items-center mb-1">
                  <span
                    className={`fw-medium ${!isCurrentMonth ? "text-muted" : ""} ${isToday ? "badge bg-primary" : ""}`}
                    style={isToday ? { borderRadius: "50%", width: "26px", height: "26px", display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: "0.8rem" } : {}}
                  >{day.getDate()}</span>
                </div>
                <div className="d-flex flex-column gap-1">
                  {appts.slice(0, 3).map((apt) => (
                    <button
                      key={apt.id}
                      className={`btn btn-sm text-start py-0 px-1 border-0 ${statusColor(apt.status)} bg-opacity-25 text-dark`}
                  ...
                  {appts.length > 3 && (
                    <span className="text-muted" style={{ fontSize: "0.7rem" }}>+{appts.length - 3} weitere</span>
                  )}
                </div>
              </div>
            );
        ...
        <div className="d-grid" style={{ gridTemplateColumns: "60px repeat(7, 1fr)", gap: "1px", background: "#e2e8f0" }}>
          ...
          {days.map((day, i) => (
            <div key={i} className={`text-center py-2 bg-white small ${day.getDate() === new Date().getDate() && day.getMonth() === new Date().getMonth() ? "text-primary fw-bold" : ""}`}>
              ...
              <div className="fw-medium">{day.getDate()}.</div>
            </div>
          ))}
          {hours.flatMap((hour) => [
              ...
              return (
                <div key={`d-${dIdx}-${hour}`} className="bg-white position-relative" style={{ minHeight: "48px", cursor: "pointer" }}>
                  {appts.map((apt) => (
                    <button
                      key={apt.id}
                      className={`btn btn-sm text-start py-0 px-1 border-0 w-100 ${statusColor(apt.status)} bg-opacity-25 text-dark`}
  ...
  const renderDay = () => {
    const appts = apptsForDay(currentDate).sort(
      (a, b) => new Date(a.startTime).getTime() - new Date(b.startTime).getTime()
    );
    return (
      <div className="bg-white rounded" style={{ border: "1px solid #e2e8f0" }}>
        <div className="p-3 border-bottom">
          <h5 className="fw-bold mb-0">{dayLabel}</h5>
        </div>
        {appts.length === 0 ? (
          <div className="p-4 text-center text-muted">
            <CalendarDays size={32} className="mb-2 opacity-50" />
            <p className="mb-0">{t("calendar.noAppointments", "Keine Termine an diesem Tag")}</p>
          </div>
        ) : (
          <div className="d-flex flex-column">
            {appts.map((apt) => (
              <button
                key={apt.id}
                className="btn text-start p-3 border-bottom w-100 d-flex align-items-center gap-3"
        ...
        <div className="card-body-custom">
          {loading ? (
            <div className="text-center py-5">
              ...
              <p className="text-muted mt-2">{t("loading.title", "Laden...")}</p>
            </div>
          ) : (
