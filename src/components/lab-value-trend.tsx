...
export default function LabValueTrend({ patientId }: Props) {
  ...
  if (loading) return <p className="text-muted" style={{ fontSize: "0.85rem" }}>Laborwerte laden...</p>;
  if (testTypes.length === 0) return <p className="text-muted" style={{ fontSize: "0.85rem" }}>Keine Laborwerte vorhanden.</p>;

  return (
    <div className="mb-3">
      <h6 className="fw-semibold mb-2" style={{ fontSize: "0.85rem" }}>Letzte Laborwerte</h6>
          ...
          <div key={type} className="d-flex align-items-center justify-content-between py-1" style={{ fontSize: "0.8rem", borderBottom: "1px solid #f1f5f9" }}>
            ...
            <span className="fw-medium">
              ...
              <span className={isHigh ? "text-danger" : isLow ? "text-warning" : "text-success"} style={{ fontSize: "0.7rem" }}>
                {isHigh ? "⚠️ hoch" : isLow ? "⚠️ niedrig" : "✓"} {trend}
              </span>
            </span>
          </div>
        );
