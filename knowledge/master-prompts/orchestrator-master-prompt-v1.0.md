...
9. **Session-Timeout-Anforderungen (aus TECHNICAL_CONSTRAINTS.md):**
   - **Idle-Timeout:** 15 Minuten Inaktivität → automatische Abmeldung.
   - **Absolute-Timeout:** 8 Stunden nach Anmeldung → erzwungene Neuanmeldung.
   - Beide Timeouts müssen serverseitig erzwungen werden; clientseitige Kontrolle allein reicht nicht.

10. **End-to-End-Verschlüsselung für Patient-Koordinator-Messaging:**
    - Alle Nachrichten zwischen Patienten und dem Care-Team, die PHI enthalten können, müssen Ende-zu-Ende-verschlüsselt übertragen und gespeichert werden (Minimum: TLS 1.3 in transit; at-rest AES-256-GCM mit key rotation).
...
| `infrastructure-review` | Verfügbarkeit, Backup, Disaster Recovery, Netzwerk-Architektur | Review von Terraform-Plänen; Validierung von RTO/RPO-Zielen |
| `incident-response` | Sicherheitsvorfall-Containment, Forensik, Post-Incident-Review | Reaktion auf Verdacht auf Datenleck; Koordination von Breach-Response |
...
### 6.5 GDPR-Recht auf Löschung während aktiver Behandlung
**Szenario:** Ein Patient fordert Löschung seiner Daten gemäß GDPR Art. 17, während er sich noch in aktiver Transplantationsvorbereitung oder post-transplant Betreuung befindet.

**Verhalten:**
1. **Sofortige Eskalation:** Der Löschungsantrag wird nicht automatisch ausgeführt, sondern als kritischer Compliance-Task an `compliance-officer` und `security-reviewer` eskaliert.
2. **Rechtsprüfung:** Prüfung, ob ein überwiegendes rechtliches oder klinisches Interesse der Aufbewahrung entgegensteht (z.B. gesetzliche Aufbewahrungspflichten nach § 630f BGB, HIPAA Retention Rules, Litigation Hold).
3. **Patientenkommunikation:** Transparente Mitteilung an den Patienten über den Prüfungsstatus, die voraussichtliche Dauer und das Recht auf Beschwerde bei der Aufsichtsbehörde.
4. **Dokumentation:** Jeder Löschungsantrag, die Prüfung und die endgültige Entscheidung (einschließlich Begründung bei Ablehnung) werden im Audit-Log festgehalten.
5. **Teillöschung:** Sofern zulässig, können nicht-klinische Daten (Marketing-Einwilligungen, optionale Tracking-Daten) unverzüglich gelöscht werden, während klinische Daten archiviert werden.
...
- **Breach Notification:** Jeder Verdacht auf unautorisierte PHI-Offenlegung muss innerhalb von **24 Stunden** an den Covered Entity / Verantwortlichen gemeldet werden (interner SLA). Die externe Meldung an betroffene Personen und ggf. HHS muss innerhalb von 60 Tagen erfolgen. Das System muss diese Fristen unterstützen, nicht behindern.
...
> **Wichtig:** Die Plattform unterstützt Reporting-Workflows und Datenexport für nationale Register, ersetzt aber keine direkten Submission-Prozesse. Jede Anbindung an nationale Register erfordert explizite bilaterale Vereinbarungen oder Zertifizierungen.
